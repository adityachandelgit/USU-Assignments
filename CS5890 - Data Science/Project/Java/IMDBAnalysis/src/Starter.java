...
public class Starter extends ApplicationFrame {
	...
	static void q3_3() {
		...
		for (int i = 0; i < lMoIn.size(); i++) {
			if(lMoIn.get(i).getTitle().equalsIgnoreCase("The Shawshank Redemption")) {
		...
		for (int i = 0; i < lMoIn.size(); i++) {
			...
			if(mppaRatingNor >= 0 && runningTimeNor >= 0 && distributorNor >= 0 && sourceNor >= 0 && majorGenreNor >= 0
				...
				//4. All 3
				//double[] currentMovie = {selectedImdbVotes, selectedImdbrating, selectedRottenRating, selectedUsGross, selectedWorldwideGross, selecteddvdSales, selectedMppa, selectedRunningTime, selectedDistributor, selectedSource,  selectedMajorGenre};
				//double[] movie = {imdbVotesNor, imdbRatingNor, rottenRatingNor, usGrossNor, worldGrossNor, dvdSaleUsdNor, mppaRatingNor, runningTimeNor, distributorNor, sourceNor, majorGenreNor};
				
				//testDistance.add(ed.compute(currentMovie, movie) + " : " + lMoIn.get(i).getTitle());
				//testDistance.add(Math.abs(currentMovie[0] - movie[0]) + Math.abs(currentMovie[1] - movie[1]) + Math.abs(currentMovie[2] - movie[2]) + Math.abs(currentMovie[3] - movie[3]) + Math.abs(currentMovie[4] - movie[4]) + Math.abs(currentMovie[5] - movie[5]) + Math.abs(currentMovie[6] - movie[6]) + Math.abs(currentMovie[7] - movie[7]) + Math.abs(currentMovie[8] - movie[8]) + Math.abs(currentMovie[9] - movie[9]) + Math.abs(currentMovie[10] - movie[10]) + " : " + lMoIn.get(i).getTitle());
				//testDistance.add(Utilities.cosineSimilarity(currentMovie, movie) + " : " + lMoIn.get(i).getTitle());
